import { K as RenderUndefinedEntryError, et as UnknownContentCollectionError, t as AstroError } from "./errors_BpmWUuHx.mjs";
import { S as unescapeHTML, b as createHeadAndContent, f as renderTemplate, n as renderScriptElement, o as renderComponent, r as renderUniqueStylesheet, t as spreadAttributes, z as generateCspDigest } from "./server_BXQXZODw.mjs";
...
var ImmutableDataStore = class ImmutableDataStore {
	...
	*/
	static async fromModule() {
		try {
			const data = await import("./_astro_data-layer-content_J2WeXDaz.mjs");
...
async function updateImageReferencesInBody(html, fileName) {
	...
	const { getImage } = await import("./_virtual_astro_get-image_D6D7eiN-.mjs");
			...
			Content: createComponent({
				factory(result, baseProps, slots) {
					...
					if (Array.isArray(collectedLinks)) links = collectedLinks.map((link) => {
						return renderUniqueStylesheet(result, {
							type: "external",
							src: (0, path_exports.isRemotePath)(link) ? link : (0, path_exports.prependForwardSlash)(link)
						});
					}).join("");
